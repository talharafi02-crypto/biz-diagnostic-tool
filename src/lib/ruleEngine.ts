...
import { TechnicalSeoResult } from "./apis/technicalSeo";
...
// ---------------------------------------------------------------------
export function scoreSeoSnapshot(ps: PageSpeedResult, site: ScrapedSite, technicalSeo: TechnicalSeoResult): ScoreCard {
  let score = 0;
  const details: string[] = [];
  ...
  if (site.title && site.title.length >= 10 && site.title.length <= 65) {
    ...
    score += 3;
    details.push(`Title tag present but poorly sized (${site.title.length} characters). Aim for 10 to 65.`);
  } else {
    details.push("Missing <title> tag. This is a basic but critical SEO gap.");
  }
  ...
  if (site.metaDescription && site.metaDescription.length >= 50) {
    ...
    details.push("Meta description missing or too short. This hurts click through from search results.");
  }

  if (site.h1Texts.length === 1) {
    score += 5;
    details.push("Exactly one H1 heading found. This is best practice.");
  ...
  // Technical crawlability: 20 points
  if (technicalSeo.checked) {
    if (technicalSeo.hasRobotsTxt) {
      score += 8;
      details.push("robots.txt found, giving search engines clear crawl instructions.");
    } else {
      details.push("No robots.txt found. Search engines fall back to default crawl behavior.");
    }

    if (technicalSeo.hasSitemap) {
      score += 8;
      details.push("XML sitemap found at /sitemap.xml, helping search engines discover all pages.");
    } else {
      details.push("No sitemap.xml found. This makes it harder for Google to discover every page on the site.");
    }

    if (technicalSeo.sitemapBlockedInRobots) {
      score -= 5;
      details.push("robots.txt appears to block the sitemap. This should be fixed immediately.");
    } else if (technicalSeo.hasRobotsTxt && technicalSeo.hasSitemap) {
      score += 4;
    }
  } else {
    score += 10; // neutral partial credit if the check itself failed to run
    details.push("Could not verify robots.txt or sitemap.xml for this run.");
  }

  // Structured data and canonical: 10 points
  if (site.hasStructuredData) {
    score += 5;
    details.push(`Structured data (schema.org) found: ${site.structuredDataTypes.join(", ") || "type not identified"}. This helps rich results in search.`);
  } else {
    details.push("No structured data (schema.org) found. Adding it can unlock rich search results like star ratings or business info.");
  }

  if (site.hasCanonical) {
    score += 5;
    details.push("Canonical tag present, reducing duplicate content risk.");
  } else {
    details.push("No canonical tag found. This is a minor gap.");
  }

  // Image accessibility, feeds into SEO too: 5 points
  if (site.imageCount > 0) {
    const altRatio = site.imagesWithAlt / site.imageCount;
    if (altRatio >= 0.8) {
      score += 5;
      details.push(`${site.imagesWithAlt} of ${site.imageCount} images have descriptive alt text.`);
    } else {
      details.push(`Only ${site.imagesWithAlt} of ${site.imageCount} images have alt text. This is a missed SEO and accessibility opportunity.`);
    }
  }

  // Lighthouse technical SEO audit: 40 points
  if (ps.checked && ps.seoScore !== null) {
    score += Math.round(ps.seoScore * 0.4);
    details.push(`Google Lighthouse technical SEO score: ${ps.seoScore}/100.`);
  } else {
    score += 20; // neutral half credit
